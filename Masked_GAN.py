...
class GANAttack:
    ...
    def train_step(self, real_images, watermark):
        ...
        for _ in range(1):
            ...
            loss_D_real = nn.BCEWithLogitsLoss()(pred_real, real_label)
            loss_D_real.backward()
            
            pred_fake = self.netD(adv_images.detach())
            fake_label = torch.zeros_like(pred_fake, device=self.device)
            loss_D_fake = nn.BCEWithLogitsLoss()(pred_fake, fake_label)
        ...
        for _ in range(1):
            ...
            loss_G_fake = nn.BCEWithLogitsLoss()(pred_fake, torch.ones_like(pred_fake, device=self.device))
